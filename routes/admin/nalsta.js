...
router.get('/', function(req, res, next) {
    if(req.session.komitent){
        if(999100000 < req.session.komitent.sifra && req.session.komitent.sifra < 999200000){
            Nalsta.findAll( { limit: 50 })
                .then(function (nalsta) {
                    if (!nalsta) {
                        req.flash('danger', 'Nepostoji ni jedna stavka!');
                        res.redirect('admin/nalsta');
                    } else {
                        res.render('admin/nalsta', { 
                            title: 'Admin', 
                ...
                .catch(function(err) { 
                    console.log(err); 
                    req.flash('danger', 'Greška u bazi!');
...
router.route('/search').post((req, res) => {
    if(req.session.komitent){
        if(999100000 < req.session.komitent.sifra && req.session.komitent.sifra < 999200000){
            var konto = req.body.konto;
        
            if (konto != "") {
                Nalsta.findAll({ where: { konto: rightPad(konto, 13) } }).then(function (nalsta) {
                    if (nalsta === undefined || nalsta.length == 0) {
                        req.flash('danger', 'Konto ne postoji!');
                        res.redirect('/admin/nalsta');
                    } else {
                        res.render('admin/resultNalsta', { 
                        title: 'Admin', 
                        naziv: req.session.komitent.naziv,
                        sifra: req.session.komitent.sifra,
                        'nalstas': nalsta
                ...
                req.flash('danger', 'Popunite konto polje!');
                res.redirect('/admin/nalsta');
            }
...
router.get('/list', function(req, res) {
    if(req.session.komitent){
        if(999100000 < req.session.komitent.sifra && req.session.komitent.sifra < 999200000){
            Nalsta.findAll().then(function (nalsta) {
                    if (!nalsta) {
                        req.flash('danger', 'Ne postoji nijedna stavka!');
                        res.redirect('admin/nalsta');
                    } else {
                        res.render('admin/listNalsta', { 
                            title: 'Admin', 
                            naziv: req.session.komitent.naziv,
                            sifra: req.session.komitent.sifra,
                            'nalstas': nalsta
    ...
    if(req.session.komitent){
        if(999100000 < req.session.komitent.sifra && req.session.komitent.sifra < 999200000){
            ...
            Nalsta.findOne({ where: { id: req.params.id } }).then(function (nalsta) {
                var konto = nalsta.konto;
                var sifra = konto.replace(/\s/g, '').slice(5);
                Komitent.findOne({ where: { sifra: rightPad(sifra, 10) } }).then(function (komitent) {
                    if (komitent.naziv != null && nalsta.vrsta == 'D'){
                        ...
                        doc.image(svg_string, 250, 300, { fit: [80, 80] });
                        ...
                        req.flash('danger', 'Greska!'); // Komitent neve nincs meg
                        res.redirect('/admin/index');
                    } 
                });
            });
